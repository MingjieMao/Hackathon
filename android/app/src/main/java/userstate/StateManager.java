...
public class StateManager {
	...
	public static boolean login(String username, String password) {
		...
		dao.model.User user = dao.UserDAO.getInstance().login(username, password);
		if (user == null) return false;

		if (user.role() == dao.model.User.Role.Admin) {
			state = new AdminState(user);
		} else {
			state = new MemberState(user);
		}
		return true;
	}
