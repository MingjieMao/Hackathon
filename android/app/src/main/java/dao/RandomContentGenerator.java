...
public class RandomContentGenerator {
	...
	public static void generatePost() {
		...
		if (user == null) return;
		String postName = String.format(Locale.ROOT, chooseRandomFromArray(POST_NAMES), chooseRandomFromArray(TOPICS));
	...
	public static void generateComment(Post post) {
		...
		String content = String.format(Locale.ROOT, "Hello from %s", user.username());

		long timestamp =  System.currentTimeMillis() - random.nextInt(200000);
