...
public class TimestampFormatterTimeSinceEnglish implements TimestampFormatter {
	...
	public String format(long timestamp) {
		...
		else if (secondsAgo < 60) return String.format(Locale.ROOT, "%d seconds ago", secondsAgo);

		long minutesAgo = secondsAgo / 60;
		if (minutesAgo == 1) return "a minute ago";
		if (minutesAgo < 60) return String.format(Locale.ROOT, "%d minutes ago", minutesAgo);

		long hoursAgo = minutesAgo / 60;
		if (hoursAgo == 1) return "an hour ago";
		if (hoursAgo < 24) return String.format(Locale.ROOT, "%d hours ago", hoursAgo);

		long daysAgo = hoursAgo / 24;
		if (daysAgo == 1) return "a day ago";
		if (daysAgo < 7) return String.format(Locale.ROOT, "%d days ago", daysAgo);
