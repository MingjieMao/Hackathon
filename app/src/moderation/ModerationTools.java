...
public class ModerationTools {
    // messageId → (userId → Report); reports that have been removed are absent from the inner map
    ...
    public static Iterator<Message> getReportedMessages(String strategy, int amount) {
        ...
        while (it.hasNext()) {
            Message m = it.next();
            messageMap.put(m.id(), m);
        }

        ReportedMessageIteratorFactory factory = switch (strategy) {
            case "OLDEST" -> new OldestStrategy();
            case "MOST" -> new MostStrategy();
            default -> throw new IllegalArgumentException("strategy must be OLDEST or MOST");
        };
        return factory.create(reports, messageMap, amount);
    }
